...
def simulate_beam(grid: utils.Grid, initial: Node) -> int:
    ...
    while explore:
        ...
        explore.extend(
            filter(lambda x: x not in energized and x[0] in grid, expand_explore(grid, current))
        )

    return len({i[0] for i in energized})
...
def main():
    ...
    for i in range(grid.rows):
        ...
        max_energized = max(
            max_energized, simulate_beam(grid, ((i, grid.rows - 1), utils.CHAR_TO_DELTA["L"]))
        )
    ...
    for i in range(grid.cols):
        ...
        max_energized = max(
            max_energized, simulate_beam(grid, ((grid.cols - 1, i), utils.CHAR_TO_DELTA["U"]))
        )
