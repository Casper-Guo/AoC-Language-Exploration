...
def get_distance(
    ...
    expand_amt = len([i for i in range(*sorted_x) if i in expand_rows]) + len(
        [i for i in range(*sorted_y) if i in expand_cols]
    )
...
def main():
    ...
    part2_distance = 0
    for galaxy1, galaxy2 in itertools.combinations(galaxies, 2):
        part2_distance += get_distance(
            galaxy1, galaxy2, no_galaxy_rows, no_galaxy_cols, 1000000
        )
